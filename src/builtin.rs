...
pub type BuiltinFn = fn(&[String], &mut dyn Write, &mut dyn Write);
...
pub fn exit(args: &[String], _w: &mut dyn Write, _err: &mut dyn Write) {
...
pub fn echo(args: &[String], w: &mut dyn Write, _err: &mut dyn Write) {
    writeln!(w, "{}", args.join(" ")).unwrap();
}

pub fn type_cmd(args: &[String], w: &mut dyn Write, err: &mut dyn Write) {
    ...
    if is_builtin(target) {
        ...
        writeln!(err, "{}: not found", target).unwrap();
    }
}

pub fn pwd(_args: &[String], w: &mut dyn Write, err: &mut dyn Write) {
    match std::env::current_dir() {
        Ok(path) => writeln!(w, "{}", path.display()).unwrap(),
        Err(e) => writeln!(err, "pwd: error: {}", e).unwrap(),
    }
}

pub fn cd(args: &[String], _w: &mut dyn Write, err: &mut dyn Write) {
